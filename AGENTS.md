...
> This file is the operating contract for any Codex agent working in this repository.
...
```bash
git add .
git commit -m "chore: initial project scaffold with PRD, Codex, and MASTERPLAN"
...
The masterplan is the **single source of truth for sequencing**. This file is the source of truth for *rules*. Precedence on conflict: masterplan (sequencing) > AGENTS.md (rules) > ENGINEERPROMPT.md (kickoff).
