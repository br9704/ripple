...
- [x] D.14 — **The test suite could not be run by anyone but its author** ✅ **FIXED.** The first CI runs went red, and the cause was not CI: `src/lib/supabase.ts:7` throws at module init when `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` are absent, and `.env.local` is gitignored. Reproduced locally by moving `.env.local` aside — **`pnpm test:run` gives 318 passing and one failed file on a clean clone, not the 334 this document and the README both claim.** The same absence broke the browser gates one layer further on: a build made without those variables produces a bundle that crashes on boot, which is why CI reported "file input not present on the report flow" against a page that had never rendered. Fixed with a committed `.env.test` carrying deliberately non-resolving placeholder values (Vite loads it automatically in test mode), plus the same values as job-level `env` in the workflow. Verified both ways: 334 with `.env.local` present, 334 with it moved aside.
  - **This is the fifth instance of the same shape, and the most pointed one**, because it undermines the number this whole documentation pass is built on. Every gate had been run only ever on the machine that wrote it, with untracked local state present. A suite a stranger cannot run is not a verified suite — it is a personal one.
  - Generalises the pattern beyond vacuity: several of these could only have failed in a state no local gate ever occupies — a clean clone, a cold cache, a first-ever arrival. Confirmed independently by S21.14, where seven of eight routes 404'd in production while every local gate passed, because `vite preview` implements history fallback itself and a warm service worker hides the rest.
- [x] D.15 — **CI badge added** ✅ Deferred at first close because the workflow had never run and its badge would have rendered "no status". It has now run: the database job passes on a `postgres:17` service container (27 migrations, 102 assertions), and the two failing jobs were failing for the reason in D.14. Badge points at `verify.yml` on `main`, so it reports the truth in either direction rather than 404ing.
