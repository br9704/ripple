...
- [~] S6.5.2 — **Unit tests for shipped logic** (CLAUDE.md §9 backfill). **41 tests across 5 files so far** — `councilDetection` (11, incl. the overlap tie-break and order-independence regression), `mapHelpers` (10, incl. every `timeAgo` boundary), `blobToBase64` (3), `compressImage` (9, DOM-stubbed), `useSubmitReport.extractFunctionError` (8). **Remaining:** `reverseGeocode` (mocked fetch), `boundaryCache` (fake-indexeddb), `useReporterToken`, `useGeolocation` (fake timers for the 5s/8s paths), `useOfflineQueue`, `useReports`.
